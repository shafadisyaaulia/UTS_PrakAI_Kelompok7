"""
Advanced Knowledge Base Module
Modul sistem pakar yang ditingkatkan dengan fitur-fitur canggih
Mendukung fuzzy logic, rule chaining, versioning, dan analytics
"""
...
class RuleStatus(Enum):
    """Status rule dalam knowledge base"""
    ACTIVE = "active"
    INACTIVE = "inactive"
...
class ConfidenceLevel(Enum):
    """Level kepercayaan untuk diagnosis"""
    ...
    def get_level(cls, cf: float) -> str:
        """Dapatkan level berdasarkan CF value"""
        for level in cls:
...
class AdvancedKnowledgeBase:
    """
    Advanced Knowledge Base dengan fitur:
    - Rule versioning dan history
    - Fuzzy logic support
    - Rule chaining dan dependency tracking
    - Analytics dan performance metrics
    - Backup dan restore
    - Rule validation yang lebih ketat
    - Export ke multiple format
    """
    
    def __init__(self, rules_file: str = 'rules.json', backup_dir: str = 'backups'):
        """
        Inisialisasi Advanced Knowledge Base
    ...
    def load_knowledge(self):
        """Load knowledge dengan error handling yang lebih baik"""
        try:
            ...
            print(f"  → {len(self.rules)} rules | {len(self.nutrients)} nutrients")
    ...
    def save_knowledge(self, create_backup: bool = True):
        """Simpan knowledge dengan opsi backup otomatis"""
        if create_backup and self.rules_file.exists():
            self._create_backup()
        
        # Convert metadata objects to dict
        metadata_dict = {
            rule_id: asdict(meta) 
    ...
    def get_active_rules(self) -> Dict[str, Any]:
        """Dapatkan hanya rules yang aktif"""
        return {
            rule_id: rule 
            for rule_id, rule in self.rules.items()
            if self.rule_metadata[rule_id].status == RuleStatus.ACTIVE.value
        }
